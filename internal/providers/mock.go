...
func (m *Mock) ReadResource(request ReadResourceRequest) ReadResourceResponse {
	...
	// TODO: Maybe look into some way we can imitate drift from the mock
	//   provider? What if the data is written into a file that could be edited
	//   or something?
...
func (m *Mock) PlanResourceChange(request PlanResourceChangeRequest) PlanResourceChangeResponse {
	...
	if request.PriorState.IsNull() {
		...
		value, diags := mocks.FillComputedValues(request.ProposedNewState, cty.NilVal, resource.Block, mocks.UnknownFiller())
...
func (m *Mock) ApplyResourceChange(request ApplyResourceChangeRequest) ApplyResourceChangeResponse {
	...
	case "create":
		...
		mockedData := cty.NilVal // If we have no mocked data we use cty.NilVal.
		if mockedResource, exists := m.Data.Resources[request.TypeName]; exists {
			if override, ok := mockedResource.Overrides.GetOk(request.Addr); ok {
				mockedData = override.Values
			} else {
				mockedData = mockedResource.Defaults
			}
		}

		value, diags := mocks.FillComputedValues(request.PlannedState, mockedData, resource.Block, mocks.ValueFiller())
		response.Diagnostics = response.Diagnostics.Append(diags)
...
func (m *Mock) ImportResourceState(request ImportResourceStateRequest) (response ImportResourceStateResponse) {
	...
	// TODO: Find use cases for this? The existing syntax for mocks does make
	//   this possible but let's find a reason to do it first.
...
func (m *Mock) ReadDataSource(request ReadDataSourceRequest) ReadDataSourceResponse {
	...
	mockedData := cty.NilVal // If we have no mocked data we use cty.NilVal.
	if mockedDataSource, exists := m.Data.DataSources[request.TypeName]; exists {
		if override, ok := mockedDataSource.Overrides.GetOk(request.Addr); ok {
			mockedData = override.Values
		} else {
			mockedData = mockedDataSource.Defaults
		}
	}

	value, diags := mocks.FillComputedValues(request.Config, mockedData, datasource.Block, mocks.DataFiller())
	response.Diagnostics = response.Diagnostics.Append(diags)
