...
func TestTestFileRunner_transform(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"only providers in run block": {
			...
			expectedProviders: make(map[string]string),
			expectedErrors: []string{
				"Missing provider definition for bar; This provider block references a provider definition that does not exist.",
			},
		},
	...
	for name, tc := range tcs {
		t.Run(name, func(t *testing.T) {
			...
			runner := &TestFileRunner{}
			reset, diags := runner.transform(config, run, file, nil)

			var errs []string
			for _, diag := range diags {
				errs = append(errs, fmt.Sprintf("%s; %s", diag.Description().Summary, diag.Description().Detail))
			}
			if diff := cmp.Diff(errs, tc.expectedErrors); len(diff) > 0 {
